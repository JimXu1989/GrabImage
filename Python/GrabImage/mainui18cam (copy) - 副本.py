...
# class Thread2222(QThread):
#     def __init__(self):
#         super(Thread2222,self).__init__()
#     def run(self):
#         self.
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.gpio.signal_send_imgs.connect(self.send_conbine_img)
        self.gpio.signal_cancel_result.connect(self.cancel_result)
        self.gpio.signal_confirm_result.connect(self.confirm_result)
        self.gpio.signal_cap_laserimg.connect(self.trigger_laser_img)
        self.gpio.signal_cap_nolightimg.connect(self.trigger_nolight_img)
        self.gpio.signal_cap_inferredimg.connect(self.trigger_inferred_img)
    ...
    def get_real_time_img(self):
        ...
        try:
            ...
            self.gpioThreadHandle = threading.Thread(target=self.gpio.GPIO_main,
                                                  )
            self.gpioThreadHandle.setDaemon(True)
            self.gpioThreadHandle.start()
            ...
            if ret == 0:
                ...
                try:
                    ...
                    if (self.inferred_trigger):
                        ...
                        self.timer.stop()
                        if (bytesPerComponent == 3):
                            self.QImg22 = QImage(self.inferredImg.data, width, height, bytesPerLine,
                                                QImage.Format_RGB888).rgbSwapped()
                        else:
                            self.QImg22 = QImage(self.inferredImg.data, width, height, bytesPerLine,
                                                QImage.Format_Grayscale8)

                        self.pixmap2 = QtGui.QPixmap.fromImage(self.QImg22)

                        self.real_time_img_win.setPixmap(self.pixmap2)
                        
                        # realtime_update_flag = False
                    if (self.nolight_trigger):
                        current_expo = c_float()
                        ret = self.cam.MV_CC_GetFloatValue('ExposureTime', current_expo)
                        if (current_expo.value != self.inferred_exposure):
    ...
    # def update_realtime_label2(self):
    #     if self.realtime_update_flag:
    #         self.img_win.setPixmap(self.pixmap)
