...
def decode_result_state(case, resDict):
    ...
    if case == [0, 0]:
        ...
        outStateCode = 0
        outCurrState = '未知错误！'
    return outCurrState, outStateCode, TarBox, AimBoxPointX, AimBoxPointY, shortD, contourX, contourY
...
def check_resDict(clientImID):
    ...
    AimBoxPointY = resDict.get('AimBoxYcnt')
    # 生成缺省返回值
    resList = [outCurrState, outStateCode, TarBox, AimBoxPointX, AimBoxPointY, shortD, contourX, contourY]
    ...
    if lenDict == 13:
        ...
        if ImLoadFlag == '1':
            ...
            outCurrState = '文本文件中"ImReadFlag"项发生错误！'
            resList[0] = outCurrState
            print(resList[0])
    elif lenDict == 0:
        outCurrState = '文本文件缺失！'
        ...
        resList[0] = outCurrState
        print(resList[0])
    # 返回值说明：设备状态、状态编号、目标框坐标数组、瞄准框X坐标数组、瞄准框Y坐标数组、检尺径、轮廓点坐标X、轮廓点坐标Y
    return resList
