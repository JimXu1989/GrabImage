...
import judge_image_property0921
from os import listdir
import Jetson.GPIO as GPIO
import time
import RPi.GPIO as GPIO
from PCA9685 import PCA9685
import test_inferredimg0826
...
class Ui_MainWindow(QObject):
    ...
    def send_conbine_img(self):
        ...
        self.sendImg = np.hstack((self.laserImg,self.inferredImg))
        self.imgNum += 1
        ...
        if((len(X1)==2)and(len(X2)==2)):
            ...
            cv2.line(clone, (X2[0], Y2[0]), (X2[1], Y2[1]), (0, 0, 255), 4)
    ...
    def blink(self, channel):
        ...
        if (GPIO.input(channel) == GPIO.LOW):
            ...
            if (clickState == 1 and click_once == 0 and program_step == 2 and self.showEnd == 1):
        ...
        try:
            while True:
                ...
                if (program_step == 2 and print_once_flag == 1):
                    ...
                    while (True):
                        res = judge_image_property0921.is_laserImage_proper(self.laserImg)
                        if (res[0] == 1):
                            break
                        elif (res[0] == 0):
                            ...
                            if ((self.laser_exposure > 5000)):
                                ...
                                self.trigger_laser_img()
                                break
                            self.laser_exposure = max(1000, self.laser_exposure + 200)
                            ...
                            self.laser_exposure = min(5000, self.laser_exposure - 200)
                            self.trigger_laser_img()
                    ...
                    while(True):
                        res = judge_image_property0921.is_grayImage_proper(self.inferredImg)
                        if(res[0]==1):
                            break
                        elif(res[0]==0):
                            ...
                            if((self.inferred_exposure>3000)):
                                ...
                                self.trigger_inferred_img()
                                break
                            self.inferred_exposure = max(100,self.inferred_exposure+200)
                            ...
                            self.inferred_exposure = min(3000,self.inferred_exposure-200)
                            self.trigger_inferred_img()
