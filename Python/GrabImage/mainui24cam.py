...
class Ui_MainWindow(QObject):
    def setupUi(self, MainWindow):
        ...
        # 抓取相机图像的QTimer，每50ms获取一次相机采集的图片，更新实时显示窗口
        self.timer = QTimer()
        self.timer.timeout.connect(self.update_realtime_label)
        self.timer.start(50)
        # 呼吸灯的QTimer，没50ms检查一次通讯情况
        self.timer2 = QTimer()
    ...
    # 打开相机，开始实时显示，在这个函数里面开启相机抓图线程和GPIO循环线程
    ...
    def send_conbine_img(self):
        ...
        self.QImg2 = QImage(clone.data, width, height, width*3,
                            QImage.Format_RGB888).rgbSwapped()
