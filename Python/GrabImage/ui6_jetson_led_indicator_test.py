...
class Ui_MainWindow(object):
    ...
    def send_conbine_img(self):
        self.sendImgButton.setDisabled(True)
        ...
        self.nolightImg = cv2.imread('image/backImage/' + '1576391767768-1000.bmp')
        ...
        with eventlet.Timeout(time_limit,False):
            try:
                ...
                response = requests.post(self.test_url, data=img_encoded.tostring()+str(self.clientImID).encode('utf-8'), headers=self.headers, timeout=5)
                self.sendImgButton.setDisabled(False)
            except:
                self.sendImgButton.setDisabled(False)
        self.sendImgButton.setDisabled(False)
        ...
        resList = judge_result_txt0727.check_resDict(self.clientImID)
        print(resList)
