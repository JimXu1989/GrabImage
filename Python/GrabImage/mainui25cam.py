        ...
        try:
            while True:
                ...
                if (program_step == 2 and print_once_flag == 1):
                    ...
                    while(True):
                        res = test_inferredimg0826.is_grayImage_proper(self.inferredImg)
                        if(res[0]==1):
                            break
                        elif(res[0]==0):
                            if((self.inferred_exposure<500)):
                                self.inferred_exposure = 500
                                self.trigger_inferred_img()
                                break
                            if((self.inferred_exposure>5000)):
                                self.inferred_exposure=5000
                                self.trigger_inferred_img()
                                break
                            self.inferred_exposure = max(500,self.inferred_exposure+500)
                            ...
                            self.inferred_exposure = min(5000,self.inferred_exposure-500)
                            self.trigger_inferred_img()
